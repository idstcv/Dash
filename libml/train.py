...
class Model:
    def __init__(self, train_dir: str, dataset: data.DataSets, **kwargs):
        ...
        self.session = None
        self.tmp = EasyDict(print_queue=[], xe_losses=[], max_probs=[], monitor_details_queue=[], xeu_losses=[], xeu_loss_mean=0.0, gamma0_multiplier = 1.0, last_gamma_epoch = -1, cache=EasyDict())
...
class ClassifySemi(Model):
    ...
    def train_step(self, train_session, gen_labeled, gen_unlabeled,
                   labeled_num = None,
                   use_dash_policy = False,
                   append_xeu_loss = False,
                   total_train_num = None):
        x, y = gen_labeled(), gen_unlabeled()
        if not use_dash_policy:
            ...
            if append_xeu_loss:
                v = train_session.run(
                    [self.ops.train_op, self.ops.loss_xeu_vec, self.ops.update_step],
                    feed_dict={self.ops.y: y['image'],
                               self.ops.xt: x['image'],
                               self.ops.label: x['label'],
                               self.ops.xeu_losses_in: np.zeros((total_train_num - labeled_num), np.float32)
                               })
            else:
                if len(self.tmp.xeu_losses) == 0:
                    xeu_losses_in = np.zeros((total_train_num - labeled_num), np.float32)
                else:
                    xeu_losses_in = np.concatenate(self.tmp.xeu_losses, axis=0)[::-1][0: total_train_num - labeled_num]
                ...
                v = train_session.run(
                    ...
                    feed_dict={self.ops.y: y['image'],
                               self.ops.xt: x['image'],
                               self.ops.label: x['label'],
                               self.ops.xeu_losses_in: xeu_losses_in
                               })

            self.tmp.step = v[-1]
            loss_xeu_vec = v[-2]
            if append_xeu_loss:
                self.tmp.xeu_losses.append(loss_xeu_vec)
    ...
    def cache_eval(self):
        ...
        def collect_samples(dataset, name):
            ...
            while 1:
                ...
                if "precise_labels" in v:
                    precise_labels.append(v['precise_labels'])
                if "noisy_labels" in v:
                    noisy_labels.append(v['noisy_labels'])
                pbar.update()
            ...
            pbar.close()
            if len(precise_labels) == 0:
                return images, labels
            else:
                precise_labels = np.concatenate(precise_labels, axis=0)
                noisy_labels = np.concatenate(noisy_labels, axis=0)
                return images, precise_labels, noisy_labels
        ...
        if 'test' not in self.tmp.cache:
            ...
            if self.dataset.train_labeled is not None:
